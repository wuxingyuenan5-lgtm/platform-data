...
MACRO_CHINA_TREASURY_ROWS = (
    ("cn_treasury_2y", "macro-cn2y", "中国 2Y", "CN2Y"),
    ("cn_treasury_10y", "macro-cn10y", "中国 10Y", "CN10Y"),
    ("cn_treasury_30y", "macro-cn30y", "中国 30Y", "CN30Y"),
)
MACRO_FRED_ROWS = (
...
def build_macro_market_detail(*, root: Path | None = None) -> dict[str, object]:
    ...
    for series_id, row_id, name, symbol in (
        *MACRO_TREASURY_ROWS,
        *MACRO_CHINA_TREASURY_ROWS,
    ):
        source_path = (
            repo_root / "public" / "v1" / "macro" / "series" / f"{series_id}.json"
        )
        ...
        row_flags = list(
            dict.fromkeys([*series.qualityFlags, *metrics.pop("qualityFlags")])
        )
        rows.append(
            {
                "id": row_id,
    ...
    payload = _json_value(
        {
            ...
            "marketId": "macro",
            "status": "ready"
            if all(item.status == "ready" for item in series_documents)
            else "partial",
